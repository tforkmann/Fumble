...
let testDatabase testName f =
    ...
    <| fun _ ->
        ...
        f @"Data Source=C:\datalog\database\datarecords.db"
...
let createStringParameterList (parameterName, value: byte, iteration) =
    let parameter = SqliteParameter()
    parameter.DbType <- DbType.String
    parameter.Value <- value
    sprintf "@%s%i" parameterName iteration, parameter
let createStringParameterList (parameterName, value: byte, iteration) =
    let parameter = SqliteParameter()
    parameter.DbType <- DbType.String
    parameter.Value <- value
...
let tests =
    testList "Fumble"
        [
          testDatabase "Create in-memory table"
          <| fun connectionStringMemory ->
              ...
              | otherwise ->
                ...
                fail ()
          testDatabase "Add records into in-memory table"
          <| fun connectionStringMemory ->
              let parameters =
                trades
                |> List.map(fun x ->
                        Sqlite.int64OrNone ("atyp", byte atyp, i))
              connectionStringMemory
              |> Sqlite.connect
              |> Sqlite.command
              ...
              | otherwise ->
                printfn "error %A" otherwise
                fail ()
        //   testDatabase "Reading a simple query"

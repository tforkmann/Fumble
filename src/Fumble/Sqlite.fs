...
module Sqlite =
    ...
    let existingConnection (connection: SqliteConnection) = { defaultProps() with ExistingConnection = connection |> Option.ofObj }
        ...
        try
            ...
            try
                ...
                for (query, parameterSets) in queries do
                    ...
                    then
                       use command = new SqliteCommand(query, connection, transaction)
    ...
    let execute (read: SqliteRowReader -> 't) (props: SqlProps) : Result<'t list, exn> =
        try
            ...
            try
                if not (connection.State.HasFlag ConnectionState.Open)
                then connection.Open()
                use command = new SqliteCommand(props.SqlQuery.Value, connection)
